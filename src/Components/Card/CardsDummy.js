
import modelo1 from '../../assets/webp/modelo1.webp'
import modelo2 from '../../assets/webp/modelo2.webp'
import modelo3 from '../../assets/webp/modelo3.webp'
import modelo4 from '../../assets/webp/modelo4.webp'
import modelo5 from '../../assets/webp/modelo5.webp'
import modelo6 from '../../assets/webp/modelo6.webp'
import modelo7 from '../../assets/webp/modelo7.webp'
import modelo8 from '../../assets/webp/modelo8.webp'
import modelo9 from '../../assets/webp/modelo9.webp'
import modelo10 from '../../assets/webp/modelo10.webp'
import modelo11 from '../../assets/webp/modelo11.webp'


export const card = [
  {
    titulo: "Modelo 1",
    precio: "1.895",
    imagen: modelo1,
    texto: "Vivienda de 200 m2. Un proyecto 3D en arquitectura para hacerte una idea aproximada de las características de la construcción, desde la geometría hasta elementos particulares, volúmenes, iluminación y también materiales constructivos. Así, reducimos la posibilidad de errores a niveles estéticos o incluso funcional y dimensional, un aspecto que puede significar un ahorro de costes. El diseño 3D permite recrear no solo el aspecto del inmueble, sino de su entorno para que la imagen sea lo más fidedigna posible respecto a lo que será una vez materializada. Permiten mostrar una imagen muy realista del resultado final del proyecto. Esa imagen es capaz de recrear ambientes y de transmitir aspectos importantes, como colores o volúmenes. Elige cambios en el mobiliario o su distribución, así como añadir elementos o modificarlos para hacerte una idea lo más aproximada posible a lo que será la imagen del proyecto una vez ejecutado.",
    id: 1
  },
  {
    titulo: "Modelo 2",
    precio: "2.395",
    imagen: modelo2,
    texto: "Vivienda de 270 m2. Un proyecto 3D en arquitectura para hacerte una idea aproximada de las características de la construcción, desde la geometría hasta elementos particulares, volúmenes, iluminación y también materiales constructivos. Así, reducimos la posibilidad de errores a niveles estéticos o incluso funcional y dimensional, un aspecto que puede significar un ahorro de costes. El diseño 3D permite recrear no solo el aspecto del inmueble, sino de su entorno para que la imagen sea lo más fidedigna posible respecto a lo que será una vez materializada. Permiten mostrar una imagen muy realista del resultado final del proyecto. Esa imagen es capaz de recrear ambientes y de transmitir aspectos importantes, como colores o volúmenes. Elige cambios en el mobiliario o su distribución, así como añadir elementos o modificarlos para hacerte una idea lo más aproximada posible a lo que será la imagen del proyecto una vez ejecutado.",
    id: 2
  },
  {
    titulo: "Modelo 3",
    precio: "2.195",
    imagen: modelo3,
    texto: "Vivienda de 260 m2. Un proyecto 3D en arquitectura para hacerte una idea aproximada de las características de la construcción, desde la geometría hasta elementos particulares, volúmenes, iluminación y también materiales constructivos. Así, reducimos la posibilidad de errores a niveles estéticos o incluso funcional y dimensional, un aspecto que puede significar un ahorro de costes. El diseño 3D permite recrear no solo el aspecto del inmueble, sino de su entorno para que la imagen sea lo más fidedigna posible respecto a lo que será una vez materializada. Permiten mostrar una imagen muy realista del resultado final del proyecto. Esa imagen es capaz de recrear ambientes y de transmitir aspectos importantes, como colores o volúmenes. Elige cambios en el mobiliario o su distribución, así como añadir elementos o modificarlos para hacerte una idea lo más aproximada posible a lo que será la imagen del proyecto una vez ejecutado.",
    id: 3
  },
  {
    titulo: "Modelo 4",
    precio: "2.895",
    imagen: modelo4,
    texto: "Vivienda de 300 m2. Un proyecto 3D en arquitectura para hacerte una idea aproximada de las características de la construcción, desde la geometría hasta elementos particulares, volúmenes, iluminación y también materiales constructivos. Así, reducimos la posibilidad de errores a niveles estéticos o incluso funcional y dimensional, un aspecto que puede significar un ahorro de costes. El diseño 3D permite recrear no solo el aspecto del inmueble, sino de su entorno para que la imagen sea lo más fidedigna posible respecto a lo que será una vez materializada. Permiten mostrar una imagen muy realista del resultado final del proyecto. Esa imagen es capaz de recrear ambientes y de transmitir aspectos importantes, como colores o volúmenes. Elige cambios en el mobiliario o su distribución, así como añadir elementos o modificarlos para hacerte una idea lo más aproximada posible a lo que será la imagen del proyecto una vez ejecutado.",
    id: 4
  },
  {
    titulo: "Modelo 5",
    precio: "2.995",
    imagen: modelo5,
    texto: "Vivienda de 320 m2. Un proyecto 3D en arquitectura para hacerte una idea aproximada de las características de la construcción, desde la geometría hasta elementos particulares, volúmenes, iluminación y también materiales constructivos. Así, reducimos la posibilidad de errores a niveles estéticos o incluso funcional y dimensional, un aspecto que puede significar un ahorro de costes. El diseño 3D permite recrear no solo el aspecto del inmueble, sino de su entorno para que la imagen sea lo más fidedigna posible respecto a lo que será una vez materializada. Permiten mostrar una imagen muy realista del resultado final del proyecto. Esa imagen es capaz de recrear ambientes y de transmitir aspectos importantes, como colores o volúmenes. Elige cambios en el mobiliario o su distribución, así como añadir elementos o modificarlos para hacerte una idea lo más aproximada posible a lo que será la imagen del proyecto una vez ejecutado.",
    id: 5
  },
  {
    titulo: "Modelo 6",
    precio: "3.495",
    imagen: modelo6,
    texto: "Vivienda de 520 m2. Un proyecto 3D en arquitectura para hacerte una idea aproximada de las características de la construcción, desde la geometría hasta elementos particulares, volúmenes, iluminación y también materiales constructivos. Así, reducimos la posibilidad de errores a niveles estéticos o incluso funcional y dimensional, un aspecto que puede significar un ahorro de costes. El diseño 3D permite recrear no solo el aspecto del inmueble, sino de su entorno para que la imagen sea lo más fidedigna posible respecto a lo que será una vez materializada. Permiten mostrar una imagen muy realista del resultado final del proyecto. Esa imagen es capaz de recrear ambientes y de transmitir aspectos importantes, como colores o volúmenes. Elige cambios en el mobiliario o su distribución, así como añadir elementos o modificarlos para hacerte una idea lo más aproximada posible a lo que será la imagen del proyecto una vez ejecutado.",
    id: 6
  },
  {
    titulo: "Modelo 7",
    precio: "4.895",
    imagen: modelo7,
    texto: "Vivienda de 620 m2. Un proyecto 3D en arquitectura para hacerte una idea aproximada de las características de la construcción, desde la geometría hasta elementos particulares, volúmenes, iluminación y también materiales constructivos. Así, reducimos la posibilidad de errores a niveles estéticos o incluso funcional y dimensional, un aspecto que puede significar un ahorro de costes. El diseño 3D permite recrear no solo el aspecto del inmueble, sino de su entorno para que la imagen sea lo más fidedigna posible respecto a lo que será una vez materializada. Permiten mostrar una imagen muy realista del resultado final del proyecto. Esa imagen es capaz de recrear ambientes y de transmitir aspectos importantes, como colores o volúmenes. Elige cambios en el mobiliario o su distribución, así como añadir elementos o modificarlos para hacerte una idea lo más aproximada posible a lo que será la imagen del proyecto una vez ejecutado.",
    id: 7
  },
  {
    titulo: "Modelo 8",
    precio: "3.095",
    imagen: modelo8,
    texto: "Vivienda de 320 m2. Un proyecto 3D en arquitectura para hacerte una idea aproximada de las características de la construcción, desde la geometría hasta elementos particulares, volúmenes, iluminación y también materiales constructivos. Así, reducimos la posibilidad de errores a niveles estéticos o incluso funcional y dimensional, un aspecto que puede significar un ahorro de costes. El diseño 3D permite recrear no solo el aspecto del inmueble, sino de su entorno para que la imagen sea lo más fidedigna posible respecto a lo que será una vez materializada. Permiten mostrar una imagen muy realista del resultado final del proyecto. Esa imagen es capaz de recrear ambientes y de transmitir aspectos importantes, como colores o volúmenes. Elige cambios en el mobiliario o su distribución, así como añadir elementos o modificarlos para hacerte una idea lo más aproximada posible a lo que será la imagen del proyecto una vez ejecutado.",
    id: 8
  },
  {
    titulo: "Modelo 9",
    precio: "4.265",
    imagen: modelo9,
    texto: "Vivienda de 420 m2. Un proyecto 3D en arquitectura para hacerte una idea aproximada de las características de la construcción, desde la geometría hasta elementos particulares, volúmenes, iluminación y también materiales constructivos. Así, reducimos la posibilidad de errores a niveles estéticos o incluso funcional y dimensional, un aspecto que puede significar un ahorro de costes. El diseño 3D permite recrear no solo el aspecto del inmueble, sino de su entorno para que la imagen sea lo más fidedigna posible respecto a lo que será una vez materializada. Permiten mostrar una imagen muy realista del resultado final del proyecto. Esa imagen es capaz de recrear ambientes y de transmitir aspectos importantes, como colores o volúmenes. Elige cambios en el mobiliario o su distribución, así como añadir elementos o modificarlos para hacerte una idea lo más aproximada posible a lo que será la imagen del proyecto una vez ejecutado.",
    id: 9
  },
  {
    titulo: "Modelo 10",
    precio: "1.965",
    imagen: modelo10,
    texto: "Vivienda de 220 m2. Un proyecto 3D en arquitectura para hacerte una idea aproximada de las características de la construcción, desde la geometría hasta elementos particulares, volúmenes, iluminación y también materiales constructivos. Así, reducimos la posibilidad de errores a niveles estéticos o incluso funcional y dimensional, un aspecto que puede significar un ahorro de costes. El diseño 3D permite recrear no solo el aspecto del inmueble, sino de su entorno para que la imagen sea lo más fidedigna posible respecto a lo que será una vez materializada. Permiten mostrar una imagen muy realista del resultado final del proyecto. Esa imagen es capaz de recrear ambientes y de transmitir aspectos importantes, como colores o volúmenes. Elige cambios en el mobiliario o su distribución, así como añadir elementos o modificarlos para hacerte una idea lo más aproximada posible a lo que será la imagen del proyecto una vez ejecutado.",
    id: 10
  },
  {
    titulo: "Modelo 11",
    precio: "3.495",
    imagen: modelo11,
    texto: "Vivienda de 320 m2. Un proyecto 3D en arquitectura para hacerte una idea aproximada de las características de la construcción, desde la geometría hasta elementos particulares, volúmenes, iluminación y también materiales constructivos. Así, reducimos la posibilidad de errores a niveles estéticos o incluso funcional y dimensional, un aspecto que puede significar un ahorro de costes. El diseño 3D permite recrear no solo el aspecto del inmueble, sino de su entorno para que la imagen sea lo más fidedigna posible respecto a lo que será una vez materializada. Permiten mostrar una imagen muy realista del resultado final del proyecto. Esa imagen es capaz de recrear ambientes y de transmitir aspectos importantes, como colores o volúmenes. Elige cambios en el mobiliario o su distribución, así como añadir elementos o modificarlos para hacerte una idea lo más aproximada posible a lo que será la imagen del proyecto una vez ejecutado.",
    id: 11
  }
]
